...
function newGame() {
  ...
  enemies = [];
  score = 0;
  txtScrore.innerText = `SCRORE: ${score}`;
  loop();
  spawnEnemies();
  ctx.fillStyle = "white";
  ctx.fillRect(0, 0, cnv.width, cnv.height);
...
function checkProjecttiles() {
  for (let i = projectiles.length - 1; i >= 0; i--) {
    ...
    for (let eIndex = enemies.length - 1; eIndex >= 0; eIndex--) {
      ...
      // colisao do projetio com o inimigo
      if (distance < p.radius + enemy.radius) {
        ...
        txtScrore.innerText = `SCORE: ${score}`;
